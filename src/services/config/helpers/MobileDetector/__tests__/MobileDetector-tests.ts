...
describe('Test client device detection', () => {
  test('Should not detect mobile when client is not mobile', () => {
    userAgents.desktop.forEach(userAgent => {
      expect((new MobileDetector(userAgent)).isClientDeviceMobile()).toBe(false);
    });
  });

  test('Should detect mobile when client is mobile', () => {
    userAgents.mobile.forEach(userAgent => {
      expect((new MobileDetector(userAgent)).isClientDeviceMobile()).toBe(true);
    });
  });
});
